...
Voor de analyse worden twee datasets gebruikt die representatief zijn voor de werklast van een mod-launcher:
...
Voor elke run worden volgende metrieken vastgelegd:
...
De genormaliseerde weergave laat een ander beeld zien dan de procentuele CPU-grafiek. Bij de mods is de volledige methode de duurste in termen van totale CPU-werklast: elke back-up kost circa 7--8 CPU-eenheden. De incrementele methode toont een sterke verbetering bij opeenvolgende incrementen: \texttt{mod\_2} en \texttt{mod\_3} kosten slechts $\sim$0{,}5--0{,}7 eenheden, doordat die operaties in minder dan één seconde afhandelen. De snapshotmethode is in beide fasen de goedkoopste: de combinatie van een korte duur en een laag CPU-percentage resulteert in een verwaarloosbare totale CPU-last per back-up- of restore-actie.


\section{Kleine mods met veel bestanden}
\label{sec:kleine-mods}

\subsection{Back-up- en restoretijd}

Figuur~\ref{fig:small-mod-time} toont de back-up- en restoretijd per back-uppunt voor de kleine mods.

\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{figures/fig_small_mod_time_per_point.png}
    \caption{Back-uptijd (links) en restoretijd (rechts) per back-uppunt voor kleine mods met veel bestanden (gemiddelde over 10 runs, foutbalken: standaardafwijking).}
    \label{fig:small-mod-time}
\end{figure}

\paragraph{Volledige back-up.}
De volledige back-up duurt $\sim$0{,}19~s, wat hoger ligt dan verwacht voor een dataset van $\sim$46~MB. Ter vergelijking duurt een volledige back-up van de savegames, die $\sim$70--144~MB groot zijn, slechts $\sim$0{,}10--0{,}17~s. De verklaring ligt in de per-bestandsoverhead: de kleine mods bestaan uit duizenden kleine bestanden. Voor elk bestand moet het systeem een afzonderlijk inode uitlezen, de bestandsattributen opvragen en een \textit{tar}-header aanmaken. Deze metadata-operaties domineren de totale verwerkingstijd ten opzichte van de effectief over te schrijven databytes, waardoor een dataset van $\sim$46~MB met veel kleine bestanden trager te verwerken is dan een dataset van het dubbele volume die uit slechts enkele grote bestanden bestaat.

\paragraph{Incrementele back-up.}
De incrementele methode toont het meest opvallende patroon: het basisarchief (\texttt{mod\_1}) duurt $\sim$0{,}06~s, maar \texttt{mod\_2} reduceert tot slechts $\sim$0{,}03~s en \texttt{mod\_3} tot $\sim$0{,}02~s. Dit weerspiegelt het feit dat opeenvolgende versies van kleine mods typisch slechts een handvol bestanden wijzigen: het overeenkomstige incrementarchief bevat nauwelijks data ($\sim$2{,}9~MB voor \texttt{mod\_2} en $\sim$245~KB voor \texttt{mod\_3}).

\paragraph{Snapshot.}
De snapshotmethode vertoont, net als bij de andere datasets, een nagenoeg constante en minimale back-up- en restoretijd ($\sim$10~ms per restore). Bij de kleine mods is de exclusieve opslaggrootte van elke snapshot nul: alle datablokken blijven gedeeld tussen de snapshot en het bronvolume. Dit duidt erop dat de wijzigingen tussen de drie modversies zo klein zijn dat ze op btrfs-blokniveau geen exclusieve blokken opleveren.

\subsection{Opslaggebruik}

Tabel~\ref{tab:small-mod-storage} vat het opslaggebruik samen voor de kleine mods, geaggregeerd over de drie back-uppunten.

\begin{table}[h]
    \centering
    \caption{Opslaggebruik voor kleine mods (drie back-uppunten, totaal). Opslag in MB.}
    \label{tab:small-mod-storage}
    \small
    \begin{tabular}{lrr}
        \toprule
        \textbf{Methode} & \textbf{Opslag (MB)} & \textbf{Besparing t.o.v. volledig} \\
        \midrule
        Volledig      & $\sim$145 & -- \\
        Incrementeel  & $\sim$49  & 66\% \\
        Differentieel & $\sim$52  & 64\% \\
        Snapshot      & $<$1      & $>$99\% \\
        \bottomrule
    \end{tabular}
\end{table}

De incrementele strategie levert bij kleine mods een bijzonder gunstige verhouding op: drie back-uppunten kosten samen slechts $\sim$49~MB, tegenover $\sim$145~MB voor drie volledige back-ups. De snapshotmethode is in dit geval nog extremer efficiënt, met een bijna verwaarloosbare fysieke voetafdruk, doordat de versies slechts minimaal van elkaar verschillen op blokniveau.

\subsection{Compressie}

Voor compressie volgen kleine mods een patroon dat zich situeert tussen savegames en grote mods. Figuur~\ref{fig:small-mod-compression} toont de archiefgrootte en compressietijd per algoritme.

\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{figures/fig_small_mod_compression.png}
    \caption{Links: archiefgrootte per compressie-algoritme voor de volledige back-up van de drie kleine mods (origineel: $\sim$141~MB). Rechts: compressietijd (log-schaal). \texttt{zstd} combineert een lage archiefgrootte met de kortste back-uptijd.}
    \label{fig:small-mod-compression}
\end{figure}

De compressieratio's liggen tussen 0{,}24 (\texttt{xz}/\texttt{lzma}/\texttt{lzip}) en 0{,}50 (\texttt{lzop}), wat lager ligt dan bij grote mods (0{,}37--0{,}54) maar hoger dan bij savegames (0{,}05--0{,}19). Dit bevestigt dat kleine mods een mix van comprimeerbare en moeilijk comprimeerbare bestanden bevatten. \texttt{zstd} comprimeert in $\sim$0{,}59~s tot $\sim$45~MB en decomprimeert in $\sim$0{,}28~s, vergelijkbaar snel als de ongecomprimeerde back-up ($\sim$0{,}60~s). De aanbeveling van \texttt{zstd} als standaardkeuze geldt daarmee ook voor kleine mods.
...
\begin{itemize}
    ...
    \item \textbf{Blokniveau-deduplicatie van btrfs.} Voor de drie mods samen bedraagt de fysieke voetafdruk van de snapshot-set $\sim$2{,}07~GB, terwijl de incrementele tar-strategie $\sim$2{,}68~GB nodig heeft. Btrfs slaagt erin op blokniveau efficiënter te dedupliceren dan tar op bestandsniveau. Bij de saves is het verschil nog meer uitgesproken ($\sim$112~MB tegenover $\sim$196~MB).
\end{itemize}
...
\subsection{Aanbeveling voor kleine mods}
